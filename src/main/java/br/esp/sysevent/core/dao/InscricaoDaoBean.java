...
//                , Order.asc("pessoa.nome"));
    ...
    protected Long atualizaInscricao(final InscricaoCommand inscricaoCmd) {
        ...
        atualizaDocumentos(inscricaoCmd.getInscricao());
        atualizaInfoSaude(inscricaoCmd.getInscricao());
        //fazer isso no controler da inscrição
    ...
    protected synchronized void ocupaVaga(final Inscricao inscricao) {
    ...
    protected synchronized void atualizaVagaOficina(final Inscricao inscricao, final Inscricao inscricaoAtual) {
        final Confraternista confraternista = inscricao.getConfraternista();
        final Confraternista confraternistaAtual = inscricaoAtual.getConfraternista();
        Oficina oficina = confraternista.getOficina();
        Oficina oficinaAtual = confraternistaAtual.getOficina();
        if (oficinaAtual != null && inscricaoAtual.isOcupaVagaGrupoOficina()) {
            oficinaAtual = oficinaDao.findById(oficinaAtual.getId());
            oficinaAtual.desocupaVaga();
            oficinaDao.saveOrUpdate(oficinaAtual);
        }
        if (oficina != null && inscricao.isOcupaVagaGrupoOficina()) {
            oficina = oficinaDao.findById(oficina.getId());
            oficina.ocupaVaga();
            oficinaDao.saveOrUpdate(oficina);
        }
    ...
    protected synchronized void atualizaGrupoIdade(Inscricao inscricao, Inscricao inscricaoAtual) {
        ...
        final Confraternista confraternista = inscricao.getConfraternista();
        final Confraternista confraternistaAtual = inscricaoAtual.getConfraternista();
        GrupoIdade grupoIdadeAtual = inscricaoAtual.getConfraternista().getGrupoIdade();
        if (grupoIdadeAtual != null && inscricaoAtual.isOcupaVagaGrupoOficina()) {
            if (!CalendarUtils.truncatedEquals(dataNascimento, dataNascimentoAtual, Calendar.DAY_OF_MONTH)
                ...
                grupoIdadeAtual = grupoIdadeDao.findById(grupoIdadeAtual.getId());
    ...
    protected synchronized void insereGrupoIdade(Inscricao inscricao, Boolean atualiza) {
        final Confraternista confraternista = inscricao.getConfraternista();
        ...
        if (gruposIdade != null && gruposIdade.size() > 0) {
            for (GrupoIdade grupoIdade : gruposIdade) {
                if (grupoIdade.getSaldoVagas() == 0) {
                    ...
                    grupoIdade = grupoIdadeDao.findById(grupoIdade.getId());
